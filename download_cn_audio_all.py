...
for id_row, content_row in cn_650.iterrows():
    ...
    working_dir = Path('cn_audio') / str(id_row+1)
    ...
    with open(working_dir / 'cn_audio.log', 'w') as log_file:
        subprocess.run(['python', '/home/zguo/Projects/jpzh_music/download_cn_audio.py', search_term], cwd=working_dir, stdout=sys.stdout, stderr=log_file)
    if id_row >= 260:
        break